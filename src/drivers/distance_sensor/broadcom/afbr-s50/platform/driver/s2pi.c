...
static inline void S2PI_InitDma(void)
{
	...
	DMA_ConfigTransfer(DMA_CHANNEL_SPI_TX, 1, DMA_MEMORY_TO_PERIPHERAL, 0, (uint32_t)(&S2PI->DL),
			   0); /* dest is data register */
	DMA_ConfigTransfer(DMA_CHANNEL_SPI_RX, 1, DMA_PERIPHERAL_TO_MEMORY, (uint32_t)(&S2PI->DL), 0,
			   0); /* src is data register */
