...
status_t UART2_Init(void)
{
	...
	if (!isInitialized2) {
		...
		DMA_ConfigTransfer(DMA_CHANNEL_UART2_TX, 1, DMA_MEMORY_TO_PERIPHERAL, 0, (uint32_t)(&UART2->D),
				   0);  /* dest is data register */
...
status_t UART_Init(void)
{
	...
	if (!isInitialized) {
		...
		/* Set up this channel's control which includes enabling the DMA interrupt */
#if defined(CPU_MKL46Z256VLH4) || defined(CPU_MKL46Z256VLL4) || defined(CPU_MKL46Z256VMC4) || defined(CPU_MKL46Z256VMP4)
		DMA_ConfigTransfer(DMA_CHANNEL_UART_TX, 1, DMA_MEMORY_TO_PERIPHERAL, 0, (uint32_t)(&UART->D),
				   0);  /* dest is data register */
#elif defined (CPU_MKL17Z256VFM4)
		DMA_ConfigTransfer(DMA_CHANNEL_UART_TX, 1, DMA_MEMORY_TO_PERIPHERAL, 0, (uint32_t)(&UART->DATA),
				   0);  /* dest is data register */
#endif
...
static inline status_t vprint(const char *fmt_s, va_list *ap)
{
	...
	status_t status = UART_SendBuffer(myDebugConsole_Buffer, (size_t)(myDebugConsole_WritePtr - myDebugConsole_Buffer), 0,
					  0);
