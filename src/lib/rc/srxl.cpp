...
static uint8_t
buffer[SRXL_FRAMELEN_MAX];       /* buffer for raw srxl frame data in correct order --> buffer[0]=byte0  buffer[1]=byte1  */
...
static uint8_t decode_state_next =
	STATE_IDLE;      /* State of frame decoding thatwill be applied when the next byte from dataframe drops in  */
static uint16_t crc_fmu =
	0U;                       /* CRC calculated over payload from srxl datastream on this machine */
...
static int srxl_channels_get_v1v2(uint16_t max_values, uint8_t *num_values, uint16_t *values, bool *failsafe_state)
{
	...
	*failsafe_state = 0U;    /* this protocol version does not support failsafe information */
	...
	for (loop = 0U; loop < *num_values; loop++) {
		channel_raw_value = ((((uint32_t)buffer[loop * 2U + 1U]) & 0x0000000FU) << 8U)  | ((uint32_t)(
					    buffer[loop * 2U +
							2U]));   /* get 12bit channel raw value from srxl datastream (mask out unused bits with 0x0000000F)  */
		channels[loop] = (uint16_t)(((channel_raw_value * (uint32_t)1400U) >> 12U) + (uint32_t)
					    800U);                    /* convert raw value to servo/esc signal pulsewidth in us */
	}
		...
		bool *failsafe_state)
{
	...
	case STATE_COLLECT: /* receive all bytes. After reception decode frame and provide rc channel information to FMU   */
		...
		if (buflen == frame_len_full) {
			...
			decode_state_next =
				STATE_IDLE; /* frame data buffering and decoding finished --> statemachine not in use until new header drops is */
...
int main(int argc, const char *argv[])
{
	...
	while (true) {
		...
		int decodestatus = srxl_decode(micros64(), b, &num_values, values, sizeof(values) / sizeof(values[0]), &failsafe_state);

		switch (decodestatus) {
		case 0:
			printf("NmbrOfChannels:%u ", num_values);
			printf("Timestamp:%u: ", micros64());
			printf("channels: ");
		...
		case 8:
			printf("SRXL version not supported\n\r");
			break;
