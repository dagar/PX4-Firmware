...
bool ImuDownSampler::update(const imuSample &imu_sample_new)
{
	...
	Vector3f delta_vel = delta_R * _imu_down_sampled.delta_vel + (imu_sample_new.delta_vel + delta_R * imu_sample_new.delta_vel) * 0.5f;
	const float delta_vel_dt = _imu_down_sampled.delta_vel_dt + imu_sample_new.delta_vel_dt;
	...
	if (_accumulated_samples > 0) {
		for (int axis = 0; axis < 3; axis++) {
			if (!_imu_down_sampled.delta_vel_clipping[axis] && imu_sample_new.delta_vel_clipping[axis]
			&& (_imu_down_sampled.delta_vel_dt > 0.001f)) {
				// axis started clipping
				//  keep existing delta velocity, but scaled to full delta_vel_dt
				delta_vel(axis) = _imu_down_sampled.delta_vel(axis) * (delta_vel_dt / _imu_down_sampled.delta_vel_dt);
				delta_vel_clipping[axis] = false;

			} else if (_imu_down_sampled.delta_vel_clipping[axis] && !imu_sample_new.delta_vel_clipping[axis]
				&& (imu_sample_new.delta_vel_dt > 0.001f)) {
				// axis stopped clipping
				//  replace entire delta velocity, but scaled to full delta_vel_dt
				delta_vel(axis) = imu_sample_new.delta_vel(axis) * (delta_vel_dt / imu_sample_new.delta_vel_dt);
				delta_vel_clipping[axis] = false;
			}
		}
	}

	_imu_down_sampled.delta_vel = delta_vel;

	_imu_down_sampled.delta_vel_clipping[0] = delta_vel_clipping[0];
	_imu_down_sampled.delta_vel_clipping[1] = delta_vel_clipping[1];
	_imu_down_sampled.delta_vel_clipping[2] = delta_vel_clipping[2];
	...
	_imu_down_sampled.delta_vel_dt = delta_vel_dt;
