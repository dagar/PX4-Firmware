 ...
 *
 * Symbolic function: compute_body_vel_hx
 *
 * Args:
 *     state: Matrix25_1
 *
...
void ComputeBodyVelHx(const matrix::Matrix<Scalar, 25, 1>& state,
  ...
  if (Hx != nullptr) {
    ...
    _hx(0, 0) = -_tmp11 * state(3, 0) - _tmp2 * _tmp3 + _tmp4 * state(0, 0) + _tmp8 * state(2, 0);
    _hx(1, 0) = _tmp11 * state(0, 0) - _tmp12 * state(2, 0) - _tmp3 * _tmp7 + _tmp4 * state(3, 0);
    _hx(2, 0) = _tmp10 * _tmp3 - _tmp12 * state(3, 0) - _tmp4 * state(2, 0) + _tmp8 * state(0, 0);
    _hx(3, 0) = -2 * std::pow(state(2, 0), Scalar(2)) - 2 * std::pow(state(3, 0), Scalar(2)) + 1;
    _hx(4, 0) = _tmp6 * state(2, 0) + _tmp9 * state(3, 0);
    _hx(5, 0) = _tmp6 * state(3, 0) - _tmp9 * state(2, 0);
  }
