...
static ssize_t progmem_read(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes, FAR uint8_t *buffer)
{
...
#ifdef CONFIG_ARCH_HAVE_PROGMEM_READ
	ssize_t result = up_progmem_read(up_progmem_getaddress(startblock) + (offset & ((1 << priv->blkshift) - 1)), buffer,
					 nbytes);
	...
	const uint8_t *src = (FAR const uint8_t *)up_progmem_getaddress(startblock) + (offset & ((1 << priv->blkshift) - 1));
...
static ssize_t progmem_write(FAR struct mtd_dev_s *dev, off_t offset, size_t nbytes, FAR const uint8_t *buffer)
{
	 ...
	 */
	off_t startblock = offset >> priv->blkshift;
	ssize_t result = up_progmem_write(up_progmem_getaddress(startblock) + (offset & ((1 << priv->blkshift) - 1)), buffer,
					  nbytes);
	return result < 0 ? result : nbytes;
}
...
FAR struct mtd_dev_s *progmem_initialize(void)
{
	...
	if (!g_progmem.initialized) {
		...
		int32_t blkshift = progmem_log2(blocksize);
		...
		int32_t ersshift = progmem_log2(blocksize);
