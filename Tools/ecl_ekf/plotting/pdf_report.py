...
def create_pdf_report(ulog: ULog, output_plot_filename: str) -> None:
    ...
    try:
        sensors_status_imu = ulog.get_dataset('sensors_status_imu').data
        print('found sensor_preflight data')
    except:
        raise PreconditionError('could not find sensors_status_imu data')
    ...
    with PdfPages(output_plot_filename) as pdf_pages:
        ...
        if ('accel_inconsistency_m_s_s' in sensors_status_imu.keys()) and (
                'gyro_inconsistency_rad_s' in sensors_status_imu.keys()):
            data_plot = TimeSeriesPlot(
                sensors_status_imu, [['accel_inconsistency_m_s_s[0]'], ['gyro_inconsistency_rad_s[0]']],
